...
function psp_update_sub_task_fe() {
    ...
    $task_index        = $_POST['task_id'];
    $progress       = $_POST['progress'];
	$phases         = get_field( 'phases', $project_id );
    $cuser          = wp_get_current_user();

    if( !isset( $phases[$phase_index]['tasks'][$parent_id]['sub_task'][$task_index] ) || empty($phases[$phase_index]['tasks'][$parent_id]['sub_task'][$task_index]) ) {
        wp_send_json_error( array( 'success' => false ) );
    }

    $phases[$phase_index]['tasks'][$parent_id]['sub_task'][$task_index]['status'] = $progress;

    $parent_task_progress = psp_calculate_parent_task_progress( $phases[$phase_index]['tasks'][$parent_id] );
    $phases[$phase_index]['tasks'][$parent_id]['status'] = $parent_task_progress;

	update_field( 'phases', $phases, $project_id );

	$phase_info    = $phases[$phase_index];
	$task_info     = $phase_info['tasks'][$parent_id]['sub_tasks'][$task_index];
