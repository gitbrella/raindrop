...
jQuery(document).ready(function($) {
    ...
    $(document).on( 'submit', '.psp-frontend-form', function(e) {
        ...
        $.ajax({
            ...
            success: function( response, success ) {
                ...
                // TODO: Do a check
                psp_fe_update_phase_progress( response.data.phase_progress, response.data.phase_index );
    ...
    $('.psp-timeline .psp-timeline--marker a').click(function(e) {
        ...
        $(marker).addClass('active').prevAll('.psp-timeline--marker').addClass('active');
    ...
    $(document).on( 'click', '.js-psp-fe-add-phase', function(e) {
        ...
        $('#psp-edit-phases-modal').find('input[name="phase_id"]').val('');
...
function psp_fe_delete_task( post_id, phase_id, task_id ) {
    ...
    jQuery.ajax({
        ...
        success: function( response ) {
            ...
            jQuery(phase).find( 'li.psp-task-id-' + task_id ).slideUp('slow', function() {
...
function psp_fe_init_modals() {
    ...
    if( jQuery('body').hasClass('psp-fe-manage-template') || jQuery('body').hasClass('psp-single-psp_pages') ) {
        return;
    }
...
function psp_fe_reset_conditionals() {
     ...
     console.log('Reseting conditionals!');
