...
function psp_fe_update_task() {
	...
	), $post_id, $phases, $phase_index, $task_id, $target );
...
function psp_fe_delete_task() {
	...
	// Gates
	if( !isset($_POST['post_id']) || !isset($_POST['phase_id']) || !isset($_POST['task_id']) ) wp_send_json_error( array( 'success' => false, 'message' => __( 'No post or phase ID set', 'psp-front-edit' ) ) );
	...
	$i = 0; foreach( $phases[$phase_index]['tasks'] as $task ) {
...
function psp_fe_populate_edit_phase_modal( ) {
	...
	if( !$phase_id ) {
		wp_send_json_error( array( 'success' => false, 'message' => __( 'No phase ID sent' , 'psp-front-edit' ) ) );
	}
	...
	$return = apply_filters( 'psp_fe_update_phase_return', array(
				...
				'modify'	=>	array(
					...
					array(
						...
						'markup'	=>	'<div class="psp-h5">' . __( 'Description', 'psp-front-edit' ) . '</div>' . $description,
						'method'	=>	'html'
					),
				),
...
function psp_fe_delete_phase() {
	...
	if( !isset($_POST['post_id']) || !isset($_POST['phase_id']) ) {
		wp_send_json_error( array( 'success' => false, 'message' => __( 'No post or phase ID set', 'psp-front-edit' ) ) );
	}
