            ...
            <?php if( $shortcode ): ?>
                ...
                <tbody>
                    ...
                    if( empty( $tasks ) ): ?>
                        <tr>
                            <td colspan="4">
                                <div class="psp-notice"><?php esc_html_e( 'There are no tasks assigned to you at this time.','psp_projects' ); ?></div>
                                <br>
                            </td>
                        </tr>
                    <?php else:
                        foreach( $tasks as $task ):
							...
							$task_class = 'task-row project-' . esc_attr( $task['post_id'] ) . '-task-' . esc_attr( $task['ID'] ) . ' psp-task-id-' . $task['task_id'] . ' task-row-' . esc_attr( $task[ 'ID' ] );
                            ...
                            <tr class="<?php echo esc_attr( apply_filters( 'psp_task_classes', $task_class, $task['post_id'], $task['phase_id'], $task['ID'], $phases, $phases[ $task['phase_id'] ] ) ); ?>" data-task_index="<?php echo esc_attr( $task['ID'] ); ?>" data-task_id="<?php echo esc_attr( $task['task_id'] ); ?>" data-phase_id="<?php echo esc_attr( $phase['phase_id'] ); ?>" data-progress="<?php echo $task[ 'status' ]; ?>">
                                ...
                                <td class="psp-task-table-body">
                                    ...
                                    <span class="psp-task-breadcrumbs">
                                        <a href="<?php echo esc_url( get_the_permalink($task['post_id']) . '#phase-' . intval($task_panel_atts['phase_index'] + 1 ) ); ?>"><?php echo esc_html( $phases[$task['phase_id']]['title']); ?></a>
                                             <?php esc_html_e( ' in ', 'psp_projects' ); ?>
                                ...
                                <td class="psp-task-table-body psp-my-tasks-progress">
