...
var _getOwnPropertyDescriptor = __webpack_require__(244);
...
(function ($) {
   ...
   */
		...
		if ($('.psp-theme-template').length) {
			...
			$('body').prepend('<div id="psp-projects" class="psp-modal-wrap"></div>');

			$('.psp-modal').each(function () {

				var modal = $(this).detach();
				$('.psp-modal-wrap').prepend(modal);
			});
		...
		if ($('#psp-offcanvas-task-details').parent().attr('id') !== 'psp-projects') {
			$('#psp-offcanvas-task-details').wrap('<div id="psp-projects"></div>');
		}

		var nav_menu = $('#psp-offcanvas-menu').data('mmenu');
		$('#psp-projects .psp-sidebar-menu a').click(function (e) {
			e.preventDefault();
			nav_menu.open();
		});
		...
		if ($('#wpadminbar').length) {
			$('#psp-title').animate({ top: "32px" }, 250);
		}
		...
		$('.psp-modal-btn').click(function (e) {
			e.preventDefault();
		});

		$('[data-toggle="psp-tooltip"]').tooltip();

		$('.modal-close, .modal_close').click(function (e) {
			e.preventDefault();
		});
		/**
   ...
   */
		if ($('.psp-theme-template').length) {
			...
			if (container_width > 1079) {
				console.log('in 1080' + container_width);
   ...
   */
		...
		$(document).on('click', '.task-list-toggle', function (e) {

			e.preventDefault();

			target = jQuery(this).parent().siblings('.psp-task-list');
		...
		$(document).on('click', '.doc-list-toggle', function (e) {
		...
		function psp_modal_reset() {
			$('#psp-projects').removeClass('psp-has-modal');
			$('.psp-modal-wrap').fadeOut();
		}

		// Comment list toggls
		$(document).on('click', '.comments-list-toggle', function (e) {
		...
		$(document).on('click', '.all-checkbox', function () {
			...
			if ($(this).is(':checked')) {
				...
				$(this).parents('.psp-document-form').find('.psp-notify-list input').prop('checked', true);
				$(this).parents('.psp-document-form').find('.psp-notify-list').slideUp('fast');
			} else {
				$(this).parents('.psp-document-form').find('.psp-notify-list input').prop('checked', false);
			}
		});
		...
		$(document).on('click', '.specific-checkbox', function () {
			...
			if ($(this).is(':checked')) {
				$(this).parents('.psp-document-form').find('.psp-notify-list').slideDown('fast');
   ...
   */
		...
		$('.psp-documents-row div.psp-document').each(function () {
      ...
      */
					...
					// Update global stats
					//
					$('#psp-stat-documents .psp-h3 span').text(response.data.approved);
		...
		$('.psp-task-project').on('click', '.task-edit-link', function (e) {
			...
			e.preventDefault();
			var the_parent = $(this).parents('div.task-item');
		...
		if ($('#psp-documents-list .psp-documents-row > .psp-document').length > 5) {

			var currentPage = 0;
			var numPerPage = 5;
			var $table = $('#psp-documents-list .psp-documents-row');
			$table.bind('repaginate', function () {
				$table.find('.psp-document').hide().slice(currentPage * numPerPage, (currentPage + 1) * numPerPage).show();
			});
			$table.trigger('repaginate');
			var numRows = $table.find('.psp-document').length;
			var numPages = Math.ceil(numRows / numPerPage);
			...
			$.ajax({
				...
				success: function success(data, textStatus) {
					if (data == 'success' || textStatus == 'success') {
						...
						if ($(form).parents('.phase-comments-wrapper, .task-comments-wrapper').children('.psp-commentlist').length > 0) {

							if (typeof psp_reverse_discussion_order !== 'undefined') {
								$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').children('.psp-commentlist').prepend(data);
							} else {
								$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').children('.psp-commentlist').append(data);
							}

							$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').find('li.psp-hide').slideDown('slow');
						} else {

							$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').prepend('.psp-commentlist').append(data);

							$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').find('.psp-hide').slideDown('slow');
						}

						// Restylize the list
						$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').find('.psp-commentlist .psp-comment:even').addClass('even');

						$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').find('.psp-commentlist .psp-comment:odd').addClass('odd');

						$(document).trigger('psp-comment-added', [form]);
					} else {

						status_div.html('<div class="psp-ajax-error">' + psp_translate_comment_error + '</div>');
					}
				}
		...
		$(document).on('click', '#psp-comments a.psp-comment-reply-link', function (e) {
			...
			var respond = $('#psp-comments #psp-project-respond').detach();

			$(this).parents('.comment').append(respond);

			$('#psp-project-respond #comment_parent').val(comment_parent);
		});
		...
		$('#psp-tasks-project-live-search').fastLiveFilter('table.psp-my-tasks tbody');
	});
})(jQuery);
...
window.psp_update_total_progress = function (projectID) {
...
window.psp_update_phase_completion = function (projectID, phaseID) {
	...
	jQuery('#phase-' + phaseID + ' .psp-task-list').children('.task-item').each(function () {
...
window.pspEqualHeight = function (group) {
...
window.psp_update_my_task_count = function (project_id) {
	...
	jQuery('.psp-task-project-' + project_id + ' .task-item, .psp-single-' + project_id + ' .task-item').each(function () {
	...
	if (jQuery('body').hasClass('psp-single')) {
		...
		if (phase_id !== null) {
			...
			jQuery('.psp-single-' + project_id + ' .phase-' + phase_id + ' .task-item').each(function () {
		...
		jQuery('#psp-stat-tasks .psp-h3 span').html(total_completed_tasks);

		total_tasks = jQuery('#psp-stat-tasks .psp-h3').text().match(/[^\/]+?$/)[0].trim();
...
function psp_update_my_phase_count(project_id) {
	...
	var completed_phases = 0,
	    total_phases = jQuery('#psp-stat-phases .psp-h3').text().match(/[^\/]+?$/)[0].trim();
	...
	jQuery('.psp-single-' + project_id + ' .psp-phase').each(function (phase_index, phase) {
		...
		if (total_tasks != 0) {
			...
			jQuery(phase).find('.task-item').each(function (task_index, task) {
	...
	jQuery('#psp-stat-phases .psp-h3 span').html(completed_phases);
...
var _foundation = __webpack_require__(227);
...
var _foundation2 = __webpack_require__(233);
...
var _foundation6 = __webpack_require__(243);

var _foundation7 = __webpack_require__(95);

var _foundation8 = __webpack_require__(247);

var _foundation9 = __webpack_require__(248);

var _foundation10 = __webpack_require__(249);
...
var _foundation17 = __webpack_require__(257);

var _foundation18 = __webpack_require__(96);

var _foundation19 = __webpack_require__(258);

var _foundation20 = __webpack_require__(259);

var _foundation21 = __webpack_require__(260);
...
var _foundation2 = __webpack_require__(250);
...
(function ($) {
	...
	$(document).ready(function () {
		...
		$(document).on('click', 'a.psp-task-title', function (event) {
			...
			data['task_description'] = data['task_description'] ? data['task_description'] : '';
			...
			if (data['task_description'].length > 0) {
				$offCanvas.find('.psp-task-table-description').html(unescape(data['task_description']));
				$offCanvas.find('.psp-task-table-description').removeClass('hidden');
			} else {
				$offCanvas.find('.psp-task-table-description').addClass('hidden');
				$offCanvas.find('.psp-task-table-description').html('');
			}
		...
		$(document).on('psp-comment-added', function (event, $form) {
			...
			var $outsideOfPanelCommentCount = $('.psp-task-id-' + data['task_id'] + ' .psp-task-discussions .text');

			console.log($outsideOfPanelCommentCount);
